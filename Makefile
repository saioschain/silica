...
HTTPS_GIT := https://github.com/sai/sai.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=sai \
...
build-docker:
	...
	mkdir -p ./build/.silica
	echo '#!/usr/bin/env bash' > ./build/silica
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/silica
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/silica
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.silica:/home/sai/.silica $$IMAGE_NAME silica "$$@"' >> ./build/silica
	chmod +x ./build/silica
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/sai/sai"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/sai:Z sai/node "./silica testnet init-files --v 4 -o /sai --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\silica:/sai\Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
	@docker run --rm -v $(CURDIR)\build\node1\silica:/sai\Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
	@docker run --rm -v $(CURDIR)\build\node2\silica:/sai\Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
	@docker run --rm -v $(CURDIR)\build\node3\silica:/sai\Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
else
	@docker run --rm -v $(CURDIR)/build/node0/silica:/sai:Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
	@docker run --rm -v $(CURDIR)/build/node1/silica:/sai:Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
	@docker run --rm -v $(CURDIR)/build/node2/silica:/sai:Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
	@docker run --rm -v $(CURDIR)/build/node3/silica:/sai:Z sai/node "./silica tendermint unsafe-reset-all --home=/sai"
endif
...
PACKAGE_NAME:=github.com/sai/sai
