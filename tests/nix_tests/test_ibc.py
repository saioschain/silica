...
def test_ibc_transfer_with_hermes(ibc):
    ...
    dst_addr = ibc.chains["sai"].cosmos_cli().address("signer2")
    old_dst_balance = get_balance(ibc.chains["sai"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["sai"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("asai"))
...
def test_evmos_ibc_transfer(ibc):
    ...
    old_src_balance = get_balance(ibc.chains["sai"], src_addr, src_denom)
    ...
    wait_for_fn("balance change", check_balance_change)
    assert old_dst_balance + amt == new_dst_balance
    new_src_balance = get_balance(ibc.chains["sai"], src_addr, src_denom)
