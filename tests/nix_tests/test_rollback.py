...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:sai_6800-1-node{i}":
            ini[section].update(
                {
...
def test_rollback(custom_evmos):
    ...
    print("stop node1")
    supervisorctl(custom_evmos.base_dir / "../tasks.ini", "stop", "sai_6800-1-node1")

    print("do rollback on node1")
    cli1.rollback()

    print("switch to normal binary")
    update_node_cmd(custom_evmos.base_dir, "silica", 1)
