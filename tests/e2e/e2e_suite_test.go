...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=10000000000000000asai", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	node.SetCmd([]string{"silica", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID), fmt.Sprintf("--home=%s.evmosd", rootDir)})
	err = s.upgradeManager.RunNode(node)
